...
-   **NEW on github**: Support for **service discovery using a Bluetooth LE "beacon"** (as an alternative to Bonjour/Rendezvous DNS-SD
    ...
    below.   The beacon runs independently of UxPlay and regularly broadcasts a Bluetooth LE ("Low Energy") 47 byte packet informing nearby iOS/macOS devices of
...
This shows two devices with their MAC addresses.    Choose which to use (we will use hci0).  Reset it with
...
* **Step 2.**  (**Optional: you can  skip it if you changed byte 6 of the initial configuration message from** "``0x01``" **to** "`0x00`".)
Use HCI LE command 5 (ocf=0x0005) to
set a fixed random "advertising address" AdvAddr, which substitutes for the true MAC address of the Bluetooth device.
...
hexadecimal digit ``s`` is one of {3, 7, b, f}, which indicates a "static" random mac address that is guaranteed to not
change between device reboots.
...
This sends a 32 byte message, where the first byte is the length (here 0x0f = 15 bytes) of the significant part of
the following 31 bytes: 15 significant bytes, padded with 16 zeros to a total length of 31 bytes.   (`hcitool` requires a message padded to the full
32 bytes, but only sends the significant bytes to the Bluetooth LE stack.)

```
$  sudo hcitool -i hci0 cmd 0x08 0x0008 0x0f 0x02 0x01 0x1a 0x0b 0xff 0x4c 0x00 0x09 0x06 0x03 0x30  0xc0 0xa8 0x01 0xfd 0x00 0x00 0x00 0x00  0x00 0x00 0x00 0x00  0x00 0x00 0x00 0x00  0x00 0x00 0x00 0x00
< HCI Command: ogf 0x08, ocf 0x0008, plen 32
  0F 02 01 1A 0B FF 4C 00 09 06 03 30 C0 A8 01 FD 00 00 00 00 
  00 00 00 00 00 00 00 00 00 00 00 00 
> HCI Event: 0x0e plen 4
  02 08 20 00 
```
The only parts of this message that you must change are the four bytes 13,14,15,16, of the IPv4 address, here "` 0xc0 0xa8 0x01 0xfd `", (decimal 192 168 1 253, an IPv4 address 192.168.1.253)
...
For creating a higher-level script, it might be useful to know that the length 0F = 15 bytes advertisement consists of  two "Advertising Protocol Data Units":

* 02 01 1A : length 02 = 2 bytes, consisting of:  01 (type = Flags) data  1A  (flags: 0001 1010 )

*  0B FF 4C 00 09 06 03 30 C0 A8 01 FD:   length 0B = 11 bytes, consisting of: FF ( type = manufacturer-specific data) 4C 00  (manufacturer code =  0x004c, Apple ) manufacturer data  09 06 03 30 C0 A8 01 FD
...
XX XX XX XX  (IPv4 network address, written as four hexadecimal octets in standard order).
